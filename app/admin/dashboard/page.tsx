    ...
    <div className="flex min-h-screen bg-slate-950 text-slate-100" suppressHydrationWarning>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between min-h-[140px]">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between min-h-[140px]">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-sm font-semibold tracking-wider uppercase">Total Guru</span>
              <GraduationCap className="text-indigo-500" size={24} />
            </div>
            <p className="text-4xl font-bold tracking-tight mt-4 text-slate-100">{dashboard.totalGuru}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between min-h-[140px]">
            <div className="flex items-center justify-between">
              <span className="text-slate-400 text-sm font-semibold tracking-wider uppercase">Total Nilai Masuk</span>
              <ClipboardList className="text-indigo-500" size={24} />
            </div>
            <p className="text-4xl font-bold tracking-tight mt-4 text-slate-100">{dashboard.totalNilai}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Tabel Guru Preview */}
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="text-md font-semibold mb-3 text-slate-200">Table Data Guru</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-400 font-medium">
                  ...
                  <tbody>
                    {dashboard.guru.slice(0, 3).map((guru) => (
                      <tr key={guru.id} className="border-b border-slate-800/50">
                        <td className="py-3 px-3">{guru.id}</td>
                        <td className="py-3 px-3">{guru.nama_guru}</td>
                        <td className="py-3 px-3">{guru.mata_pelajaran}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {/* Tabel Siswa Preview */}
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
              <h3 className="text-md font-semibold mb-3 text-slate-200">Table Data Siswa</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-400 font-medium">
                  ...
                  <tbody>
                    {dashboard.siswa.slice(0, 3).map((siswa) => (
                      <tr key={siswa.nis} className="border-b border-slate-800/50">
                        <td className="py-3 px-3">{siswa.nis}</td>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 h-full min-h-[350px]">
                <h3 className="text-md font-semibold mb-4 text-slate-200 text-center">Aktivitas Terbaru</h3>
