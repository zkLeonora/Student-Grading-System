  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100">
      <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col justify-between p-4 sticky top-0 h-screen">
        <div className="space-y-6">
          ...
          <nav className="space-y-1">
            <button onClick={() => router.push("/guru/dashboard")} className="w-full flex items-center gap-3 rounded-full px-4 py-2.5 text-sm font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800/50">
              <LayoutDashboard size={18} /><span>Dashboard</span>
            </button>
            <button onClick={() => router.push("/guru/kelola-nilai")} className="w-full flex items-center gap-3 rounded-full px-4 py-2.5 text-sm font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-800/50">
              <Users size={18} /><span>Kelola Nilai Siswa</span>
            </button>
            <button className="w-full flex items-center gap-3 rounded-full px-4 py-2.5 text-sm font-medium bg-indigo-600 text-white shadow-lg shadow-indigo-600/10">
              <FileText size={18} /><span>Laporan Nilai</span>
            </button>
          </nav>
        </div>

        <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 bg-slate-800 hover:bg-red-950/40 border border-slate-700 text-slate-300 py-2 rounded-full text-sm transition">
      ...
      <main className="flex-1 p-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-100">Laporan Nilai Siswa</h1>
          <p className="text-slate-400 mt-1">Rekapitulasi seluruh nilai akhir dan status kelulusan siswa.</p>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400 uppercase text-xs tracking-wider font-bold">
                ...
                <th className="py-4 px-4 text-center">Status Nilai</th>
            ...
            <tbody>
              {semuaNilai.map((n) => (
                <tr key={n.id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                  ...
                  <td className="py-4 px-4 font-medium text-slate-200">{n.nama}</td>
